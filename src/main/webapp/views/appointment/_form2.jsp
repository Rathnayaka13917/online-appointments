    ...
    <div class="card-header border-0 pt-6 my-1">
        ...
        <div class="card-toolbar">
            <div class="d-flex justify-content-end">
                <a href="/online-appointments/appointments"><button type="button" class="btn btn-primary">Back</button></a>
            </div>
        </div>
    </div>
    <%
        ...
        <form action="<%= (action[action.length-1].equals("new"))?"new?step=2" : "update?id="+Integer.parseInt(request.getParameter("id"))+"" %>" method="post">
            ...
            <div class="form-group row mb-3">
                <div class="col-lg-6">
                    ...
                    <select class="form-select" aria-label="Default select example" id="availability" name="availability" >
                        <option value="">Select Date and Time Slots</option>
                        <% for(ConsultantAvailability obj:availability_list){ %>
                        	<option <% if(appointment!=null){ %> <%=(appointment.getAvailability()==obj.getId())?"selected":"" %> <% } %> value="<%= obj.getId() %>"><%= obj.getDay() %>(<%=obj.getStart_time()+" - "+obj.getEnd_time() %>)</option>
                        <% } %>
                    </select>
                </div>
            ...
            <div class="form-group row mb-3">
                <div class="col-lg-12">
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control" id="remarks" name="remarks" placeholder="Remarks" <% if(appointment!=null){ %> value="<%= appointment.getRemarks() %> <% } %>">
